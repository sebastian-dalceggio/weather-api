...
from soda.contracts.data_contract_translator import (  # type: ignore[import-untyped]
    DataContractTranslator,
)

from weather_api.download.smn_download import download_data
from weather_api.utils.date import get_pendulum_datetime
...
class Query(ABC):
    ...
    def check_load_to_database(
        cls, date: str, data_source_name: str, configuration_file_path: Path
    ) -> None:
        ...
        Raises:
            ValueError: if the table doesn't follow the data contract.
        """
        scan = Scan()
        ...
        scan.add_variables({"NOW": str(pendulum_date.naive())})
        ...
        if scan.has_check_fails():
            raise ValueError("Soda Scan failed with errors!")
